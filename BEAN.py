...
TIMEZONE = datetime.timezone(datetime.timedelta(hours=-7)) # PST
USER_HISTORY_FILE = "/data/user_history.json"
RNG_BALANCE_STRENGTH = 2.2
...
def get_world_records(target, players):
    ...
    for var in variable_data:
        ...
        # Cave Extensions: Mini & Max
            ...
            # Temporary fix for pilot quest being backwards??
            if game == 'Pilot Quest':
...
# randomdaily
@client.tree.command(name="randomdaily",description="Get a personalized random UFO 50 game suggestion for the day based on your username and today's date", guild=GUILD_ID)
...
async def allgames_numbered(interaction: discord.Interaction):
    games_list = "".join(f'`{i+1:02}`{d[i]["emoji"]}' for i in range(0, 10))
